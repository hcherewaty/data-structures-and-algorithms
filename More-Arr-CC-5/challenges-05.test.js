...
const listFoods = (recipe) => {
  ...
  // Solution code here...
    recipe.ingredients.forEach( (idx) => {
      let ingredientList = idx.slice(idx.indexOf(' ') +1 );
      let newIngredientList = ingredientList.slice(ingredientList.indexOf(' ') +1 );
      result.push(newIngredientList);
    })
    return result; 
  }
...
const splitFoods = (recipe) => {
  let result = [];
  // Solution code here...
  // let ingredientList = recipe.ingredients.toString();

  recipe.ingredients.forEach( (idx) => {
    let newList = idx.split(' ').slice(2).join(' ');
    // let newListTwo = newList.split(newList.indexOf(' ') +1);
    result.push(newList);
  })
