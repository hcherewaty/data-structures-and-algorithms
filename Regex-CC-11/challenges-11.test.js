...
const validateEmail = (email) => {
  // Solution code here...
  let validator = /^\w+\.\w+\@\w+(.com|.org|.net)$|^\w+\@\w+(.com|.org|.net)$/g;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let validator = /^\d{10}$|^\(\d{3}\)\s\d{3}\-\d{4}$|^\(\d{3}\)\d{3}\s\d{4}$|^\d{3}\s\d{3}\-\d{4}$|^\d{3}\-\d{7}$|^\d{3}\-\d{3}\s\d{4}$|^\d{3}\-\d{3}\-\d{4}$|^\d{3}\s\d{3}\s\d{4}$|^\d{6}\-\d{4}$|^\d{10}$|^\d{3}\s\d{7}$/g;
